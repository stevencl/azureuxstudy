...
def uploadToStorage(image_name, blob_name, frame):
    ...
    with open(image_name, 'rb') as image_file:
        container_client.upload_blob(blob_name, image_file.read(), overwrite=True)
        #blob_client.upload_blob(image_file.read())

def sendEvent(emotion_data, blob_name):
    batch = eventhub_client.create_batch(max_size_in_bytes=10000)
    
    data = EventData(emotion_data)
    data.properties["blob_name"] = blob_name
    try:
        batch.add(data)
    except ValueError:
...
def handleNewImage(image_name, frame):
    ...
    if emotions:
        ...
        blob_name = "face"+str(d)+".png"
        uploadToStorage(image_name, blob_name, frame)
        sendEvent(str(emotions), blob_name)
